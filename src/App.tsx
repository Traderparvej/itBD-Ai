    ...
    <div className="min-h-screen bg-[#050b16] text-white font-sans selection:bg-cyan-500/30">
      ...
      <div className="relative container max-w-[520px] mx-auto px-5 py-8">
        ...
        <motion.div 
          ...
          className="bg-[#111a2e]/75 backdrop-blur-[18px] rounded-[20px] p-6 shadow-[0_0_40px_rgba(0,255,200,0.08)] border border-white/5"
        >
          {/* Market Selection */}
          <div className="space-y-4">
            <div>
              <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">Market</label>
              <select 
                value={market}
                onChange={(e) => {
                ...
                className="w-full p-3.5 rounded-xl border border-white/5 bg-[#0c1628] text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500/30 transition-all appearance-none cursor-pointer"
              >
            ...
            <div className="relative">
              <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">🔍 Search Asset</label>
              <div className="relative">
                <input 
                  type="text" 
                  value={assetSearch}
                  ...
                  className="w-full p-3.5 pl-10 rounded-xl border border-white/5 bg-[#0c1628] text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500/30 transition-all"
                />
            ...
            {/* Direction */}
            <div>
              <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">Direction</label>
              <select 
                value={direction}
            ...
            {/* Timeframe */}
            <div>
              <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">Timeframe</label>
                ...
                {['M1', 'M5', 'M15', 'M30'].map((tf) => (
                  <button 
                    ...
                    className={`flex-1 p-2.5 rounded-xl border text-xs font-bold transition-all ${
                      timeframe === tf 
                        ? 'bg-gradient-to-r from-[#00ffc8] to-[#00a2ff] text-black border-transparent' 
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">Number of Signals</label>
                <input 
                  type="number" 
                  value={count}
                  onChange={(e) => setCount(Math.min(200, Math.max(1, parseInt(e.target.value) || 1)))}
                  className="w-full p-3.5 rounded-xl border border-white/5 bg-[#0c1628] text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500/30 transition-all"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">Start Time</label>
                  <input 
                    type="time" 
                    value={startTime}
                ...
                <div>
                  <label className="text-[12px] text-[#8fa3c7] mb-1.5 block">End Time</label>
                  <input 
                    type="time" 
                    value={endTime}
            ...
            <button 
              ...
              className="w-full py-4 mt-4 rounded-2xl bg-gradient-to-r from-[#00ffc8] to-[#00a2ff] text-black font-bold text-lg shadow-xl shadow-cyan-500/20 hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <div className="mt-6 space-y-3">
              <div className="bg-[#061021] rounded-xl p-4 min-h-[100px] max-h-[300px] overflow-y-auto font-mono text-xs leading-relaxed border border-white/5">
                {loadingStep ? (
                  ...
                  <pre className="whitespace-pre-wrap text-white">{getSignalMessage()}</pre>
                ) : (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-[100] p-5">
            ...
            >
              ...
              <button 
                onClick={checkAccess}
                className="w-full py-4 rounded-xl bg-gradient-to-r from-[#00ffc8] to-[#00a2ff] text-black font-bold text-lg hover:opacity-90 active:scale-[0.98] transition-all"
              >
                Unlock
              </button>
              
              <div className="mt-6 text-xs text-[#8fa3c7]">
                Access Required? Contact: <br />
                <a href="https://t.me/ITBD_Parvej" target="_blank" rel="noreferrer" className="text-[#00ffc8] font-bold hover:underline mt-1 inline-block">@ITBD_Parvej</a>
              </div>
            </motion.div>
          </div>
        )}
